...
import comfig.py as con


application = Flask(__name__)

url = con.DATABASE_URI
application.secret_key = con.SECRET_KEY
...
def landingPage():
    if request.method == 'POST':
        if login() == 'Test successful':
            return login()
    return render_template("/app/landingPage.html")
...
def profile():
    ...
    return render_template("/app/profile.html", profileResult=profileResult )
...
def profileEdit():
    ...
    return render_template("/app/profileEdit.html")#, image_file = image_file)
...
def NLP():
    ...
    return render_template("/app/NLP.html", data = "")
